...
model.add(Conv2D(kernel_size=8,strides=2,filters=30,padding="same",activation="relu",name="capa_1")) 

model.add(MaxPool2D(pool_size=2,strides=2))

model.add(Conv2D(kernel_size=8,strides=2,filters=36,padding="same",activation="relu",name="capa_2"))
model.add(MaxPool2D(pool_size=2,strides=2))
...
ruta="Alzheimer/Resources/models/modeloTres.h5"
model.save(ruta)
model.summary()
