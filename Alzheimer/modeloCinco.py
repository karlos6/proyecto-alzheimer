...
model.add(Conv2D(kernel_size=5,strides=2,filters=32,padding="same",activation="relu",name="capa_2"))
model.add(MaxPool2D(pool_size=2,strides=2))

model.add(Conv2D(kernel_size=5,strides=2,filters=64,padding="same",activation="relu",name="capa_3"))
model.add(MaxPool2D(pool_size=2,strides=2))

model.add(Conv2D(kernel_size=5,strides=2,filters=128,padding="same",activation="relu",name="capa_4"))
model.add(MaxPool2D(pool_size=2,strides=2))
...
model.fit(x=imagenes,y=probabilidades,epochs=77,batch_size=777)
...
ruta="Alzheimer/Resources/models/modeloCinco.h5"
model.save(ruta)
model.summary()
